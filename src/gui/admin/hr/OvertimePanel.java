...
public class OvertimePanel extends javax.swing.JPanel {
    ...
    public final void loadOvertimeHistory() {
        List<OvertimeRequest> overtimes = requestService.getAllOvertimeRequests();
        ...
        UIUtil.styleTable(jTableOvertimeRecords, cols);

        DefaultTableModel model = (DefaultTableModel) jTableOvertimeRecords.getModel();
        model.setRowCount(0);

        for (OvertimeRequest or : overtimes) {
        ...
        UIUtil.installSearchFilter(jTableOvertimeRecords, jTextFieldSearch, 0, 1, 2, 3, 4, 5);
    }
    
    protected void onViewClicked() {
        int row = jTableOvertimeRecords.getSelectedRow();
        if (row < 0) {
            UIUtil.showWarningMessage(this, "Please select an overtime request to view.", "No Selection");
            return;
        }

        int overtimeID = (int) jTableOvertimeRecords.getValueAt(row, 0);

        hrPortal.getViewOvertimePanel().loadSelectedOvertime(overtimeID);
    ...
    private void initComponents() {
        ...
        jTableOvertimeRecords = new javax.swing.JTable();
        ...
        jTableOvertimeRecords.setAutoCreateRowSorter(true);
        jTableOvertimeRecords.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
            ...
            new String [] {
                "Overtime ID", "Request Date", "Employee ID", "Overtime Work Date", "Overtime Hours", "Status"
            }
        ) {
        ...
        jTableOvertimeRecords.setFocusable(false);
        jTableOvertimeRecords.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        jTableOvertimeRecords.setShowGrid(true);
        jTableOvertimeRecords.getTableHeader().setResizingAllowed(false);
        jTableOvertimeRecords.getTableHeader().setReorderingAllowed(false);
        jScrollPane1.setViewportView(jTableOvertimeRecords);
    ...
    private javax.swing.JTable jTableOvertimeRecords;
